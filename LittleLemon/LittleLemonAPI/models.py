...
class Cart(models.Model):
    ...
    def save(self, *args, **kwargs):
        self.unit_price = self.menuitems.price
        self.price = self.quantity * self.unit_price
        
        super().save(*args, **kwargs)
    def __str__(self):
        return f"{self.quantity} x {self.menuitems.title} for {self.user.username}"
    class Meta:
...
class Order(models.Model):
    ...
    def __str__(self):
        return f"Order for {self.user.username} on {self.date}"
...
class OrderItem(models.Model):
    ...
    def __str__(self):
        return f"{self.quantity} x {self.menuitem.title} for {self.order.user.username}"
    class Meta:
